...
Il progetto consiste nella realizzaione in hardware di un sistema, che esegue attraverso il protocollo di handshake, utilizza due moltiplicatori in virgola mobile a precisione singola, secondo lo standard IEEE754, per eseguire due moltiplicazioni in parello. Esso deve essere sintetizzabile sulla scheda FPGA ``xc7z020clg400-1'' che possedendo solo 125 porte, obbliga a serializzare input e output.
\\Il livello superiore del sistema, chiamato \textbf{double\_multiplier}, è stato realizzato in Verilog e contiene al suo interno un moltiplicatore in Verilog, \textbf{verilog\_multiplier}, e uno in VHDL, \textbf{vhdl\_multiplier}. I moltiplicatori sono stati scritti in linguaggi diversi al fine di confrontare i due linguaggi e capirne pro e contro. L'intero sistema è stato poi tradotto in SystemC per fare un confronto anche con quel tipo di progettazione e infine si sono analizzati i risultati confrontati con quelli dell'high level syntesis del codice in c++.
\\L'approccio utilizzato è bottom-up, cioè partendo dal moltiplicatore per poi arrivare al top level. L'implementazione è preceduta dall'analisi dei requisiti e la stesura della EFSM cioè la parte più importante in quanto è dove viene tradotto l'algoritmo, descritto il flusso e scelti i vari segnali e registri necessari. Una buona EFSM permette di evitare di scrivere varie righe di codice per poi accorgersi in simulazione che qualcosa non funziona.
\\Il progetto è piccolo quindi ci si aspetta che occupi una minima parte della FPGA.
\\Ci si aspetta anche che la versione RTL sia significativamente più performante di quella con l'high level syntesis, motivo per cui ha senso usare questo tipo di approccio quando non si hanno stretti vincoli di tempo da rispettare per realizzare il progetto.
