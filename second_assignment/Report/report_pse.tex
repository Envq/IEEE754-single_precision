...
\end{itemize}
Come valore di \textbf{timing_annotation} è stato utilizzato 100ns, valore ricavato dalla moltiplicazione di 10ns, cioè il periodo minimo a cui il componente sintezzato può funzionare, per 10 cioè la lunghezza di cicli di clock media che sono necessari al componente per eseguire le due moltiplicazioni. Questo valore in base ai valori dei due operanti utilizzati può variare molto, come si può osservare lanciando la simulazione col \terxtbf{full_target_test} della descrizione in SystemC RTL, che evidenzia una certa approssimazione di questi valori.
